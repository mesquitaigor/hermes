...
import { PasswordPopupController, RulesPopupInputData } from '../../components/password-level-popup/password-level-popup.component'
...
export default class PasswordLevelPopup{
  ...
  rules: RulesPopupInputData[] = [
    {
      ...
      condition: () => this.pristineInputRuleCondition('shouldHaveUpperLetters'),
      levelText: 'Tá fraca ainda.',
    },
    {
      label: 'Possui letras mínusculas',
      condition: () => this.pristineInputRuleCondition('shouldHaveLowerLetters'),
    ...
    },
    {
      ...
      condition: () => this.pristineInputRuleCondition('shouldHavespecialCharacters'),
  ...
  setPopupController(controller: PasswordPopupController){this.controller = controller}
  getPopupController(){return this.controller}

  setPasswordInput(control: AbstractControl){this.passwordInput = control}
  ...
  pristineInputRuleCondition(errorName: string){
    return !this.passwordInput?.errors?.[errorName] && !this.passwordInput?.pristine
  }
}
